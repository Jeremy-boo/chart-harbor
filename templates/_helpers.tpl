...
{{- define "harbor.auth.enable" -}}
  {{- if or (ne .Values.Auth.mode "") (.Values.oidc.enable) -}}
    {{- printf "true" -}}
  {{- else -}}
    {{- printf "false" -}}
  {{- end -}}
...
{{- define "harbor.auth.oidcmode" -}}
  {{- if ne .Values.oidc.harbor.mode "" -}}
    {{- .Values.oidc.harbor.mode -}}
  {{- else if ne .Values.Auth.mode "" -}}
    {{- .Values.Auth.mode -}}
  {{- end -}}
{{- end -}}

{{- define "harbor.auth.harborUrl" -}}
  {{- if ne .Values.oidc.serverURL "" -}}
    {{- .Values.oidc.serverURL -}}
  {{- else if ne .Values.Auth.harborURL "" -}}
    {{- .Values.Auth.harborURL -}}
  {{- end -}}
{{- end -}}


{{- define "harbor.auth.clientId" -}}
  {{- if ne .Values.oidc.clientID "" -}}
    {{- .Values.oidc.clientID -}}
  {{- else if ne .Values.Auth.oidcClientID "" -}}
    {{- .Values.Auth.oidcClientID -}}
  {{- end -}}
{{- end -}}

{{- define "harbor.auth.clientsecret" -}}
  {{- if ne .Values.oidc.clientSecret "" -}}
    {{- .Values.oidc.clientSecret -}}
  {{- else if ne .Values.Auth.oidcClientSecret "" -}}
    {{- .Values.Auth.oidcClientSecret -}}
  {{- end -}}
{{- end -}}


{{- define "harbor.auth.issuer" -}}
  {{- if ne .Values.oidc.issuer "" -}}
    {{- .Values.oidc.issuer -}}
  {{- else if ne .Values.Auth.oidcEndpoint "" -}}
    {{- .Values.Auth.oidcEndpoint -}}
  {{- end -}}
{{- end -}}

{{- define "harbor.auth.oidcName" -}}
  {{- if ne .Values.oidc.harbor.oidcName "" -}}
    {{- .Values.oidc.harbor.oidcName -}}
  {{- else if ne .Values.Auth.oidcName "" -}}
    {{- .Values.Auth.oidcName -}}
  {{- end -}}
{{- end -}}


{{- define "harbor.auth.oidcScope" -}}
  {{- if ne .Values.oidc.scope "" -}}
    {{- .Values.oidc.scope -}}
  {{- else if ne .Values.Auth.oidcScope "" -}}
    {{- .Values.Auth.oidcScope -}}
  {{- end -}}
{{- end -}}

{{- define "harbor.auth.oidcVerifyCert" -}}
  {{- if .Values.oidc.verifyCert -}}
    {{- printf "true" -}}
  {{- else if .Values.Auth.oidcVerifyCert -}}
    {{- printf "true" -}}
  {{- else -}}
    {{- printf "false" -}}
  {{- end -}}
...
{{- define "harbor.redis.url" -}}
  {{- with .Values.redis }}
    ...
    {{- if eq .type "internal" }}
    {{- $cred := ternary (printf "redis:%s@" (.internal.password | urlquery)) "" .internal.usePassword }}
